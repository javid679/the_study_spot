...
public class CustomerDetails implements UserDetailsService {
    ...
    @Override
    @Transactional
    public UserDetails loadUserByUsername(String email)//Loading user Details by UserName
            throws UsernameNotFoundException {
    	try {
            User user = userService.findByEmail(email);   // FInding Email in DB
            return UserPrincipal.create(user);
    	}catch(Exception e) {
	   		throw new UsernameNotFoundException("User not found with Email : " + email);// Handling Excpetion
	   	}
    }
    ...
    public UserDetails loadUserById(Long id) {
    	try {
   		 User user = userService.getUserDetailById(id);
   	        return UserPrincipal.create(user);// Create 
	   	}catch(Exception e) {
