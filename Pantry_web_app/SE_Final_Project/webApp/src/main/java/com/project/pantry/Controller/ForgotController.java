/**
 * @author webpantry
 * @RequestMapping("api/forgot") API to check for forgot password 
 */
...
public class ForgotController {
	...
	public ResponseEntity<?> forgotPassword(@RequestBody HashMap<String,String> forgotRequest) {
		try {
			User user = userservice.forgotuser(forgotRequest);// Calling service to check email Exists or not
			return  ResponseEntity.ok(user);
		}catch(Exception e ) {
			return ResponseEntity.badRequest().body(new ApiResponse(e.getMessage(), ""));// Excpetion Handling
		}
	}
}
