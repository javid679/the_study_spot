/**
 * @author webpantry
 * @RequestMapping("rest-api") This is Admin Login API which fetches username and password from DB and check for credentials added by the Admin.
 */
...
public class AdminController {  
	...
	public ResponseEntity<?> adminLogin(@RequestBody LoginPojo loginRequest) throws Exception {
		...
		String password = loginRequest.getPassword();// Fetching EMail and Password
		if(adminService == null) {
			 logger.info("user found the error");
			 throw new BadCredentialsException("1001");// Handing Bad Error Excpetions
		}
		AdminLogin admin = adminService.findByEmail(email);
		if (admin == null) {
	        throw new BadCredentialsException("Admin User Not found!!");// handling Excpetions for DB Admin User not found
	  }
		 logger.info("from authentication "+password+" from db "+admin.getPassword());
		 if(!this.passwordMatch(password, admin.getPassword())) {
			 logger.info("Password is wrong for user .."+admin.getEmail()+"-- "+admin.getEmail());// Password Matching
	...
	private Boolean passwordMatch(String rawPassword,String from_db_encoded) {// method to check password
