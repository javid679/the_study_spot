/**
 * @author webpantry
 * This is AddToCart Repository to perform all DB operations
 */
...
public interface AddtoCartRepository extends JpaRepository<Cart,Long> {
	...
	List<Cart> getCartByuserId(@Param("email")String email);// Get Cart for the USER by Email
	...
	void deleteCartByIdAndUserId(@Param("email")String email,@Param("cart_id")Long cart_id);// Delete Cart by cartID
	@Modifying
    @Transactional
	...
	@Modifying
    @Transactional
	@Query("DELETE  FROM Cart addCart WHERE addCart.email=:email") // Delete Single Product Item
	void deleteAllCartUserId(@Param("email")Long email);
	@Modifying
    @Transactional
	@Query("update Cart addCart set addCart.qty=:qty WHERE addCart.id=:cart_id")// Update Qty for the product Item
